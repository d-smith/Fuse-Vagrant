...
cd $HOME
if [ -e /vagrant/xtrac-jboss-poc-1.4.zip ]
then
	cp /vagrant/xtrac-jboss-poc-1.4.zip $HOME
else
	wget http://vc2c09dal2306.fmr.com/installApp/xtrac-jboss-poc-1.4.zip
fi
...
mv xtrac-jboss-poc-1.4.zip /apps

#Prep the installApp distro
cd /apps
unzip xtrac-jboss-poc-1.4.zip
cd fuse_esb/xtrac-jboss-poc-1.4/app-bin
...
cd /apps
echo VAGRANT > envid
cd /apps/fuse_esb/xtrac-jboss-poc-1.4/app-bin
